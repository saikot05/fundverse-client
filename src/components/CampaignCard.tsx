...
export default function CampaignCard({ campaign, index = 0 }: CampaignCardProps) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className="bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800/80 hover:border-violet-500/30 shadow-lg hover:shadow-violet-500/10 transition-all duration-500 h-full flex flex-col group overflow-hidden rounded-2xl">
          ...
          <div className="mt-auto space-y-2">
            <div className="flex justify-between items-end text-xs font-semibold text-slate-500 dark:text-slate-400">
              ...
              <motion.span
                ...
                className="text-violet-600 dark:text-violet-400 font-bold bg-violet-500/10 px-2 py-0.5 rounded text-[10px]"
              >
                {percent}%
              </motion.span>
            </div>
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800/60 h-2 rounded-full overflow-hidden shadow-inner">
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.9, delay: index * 0.08 + 0.2, ease: [0.22, 1, 0.36, 1] }}
              />
            </div>
          </div>
        </Card.Content>
