...
export default function HomePage() {
  ...
  // Fetch 3 active campaigns for homepage
  const { data } = useQuery({
    queryKey: ['home-campaigns'],
    queryFn: () => campaignService.getAll({ page: 1, limit: 3, status: 'active' }),
  });
  ...
  return (
    <div className="flex-1 bg-slate-950 text-white">
      ...
      <section className="relative h-[550px] w-full overflow-hidden border-b border-white/10 flex items-center">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 w-full">
          <div className="max-w-2xl space-y-6">
            <span className="inline-flex items-center gap-1 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <Link
                href="/campaigns"
                className="rounded-xl bg-gradient-to-r from-violet-500 to-indigo-500 hover:from-violet-600 hover:to-indigo-600 px-6 py-3 font-semibold text-sm shadow-lg shadow-indigo-500/20 transition-all duration-200 flex items-center gap-2"
              >
              ...
              <Link
                ...
                className="rounded-xl border border-white/10 hover:border-indigo-500 bg-white/5 hover:bg-indigo-500/5 px-6 py-3 font-semibold text-sm text-slate-300 hover:text-white transition"
              >
                Launch Idea
              </Link>
            </div>
            ...
            {heroSlides.map((_, idx) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${
                  heroIndex === idx ? 'w-8 bg-indigo-500' : 'w-2.5 bg-slate-700 hover:bg-slate-600'
                }`}
              />
            ))}
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="glass p-6 rounded-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-xl font-bold text-white">45,000+ Backers</p>
              <p className="text-xs text-slate-400">Supporting global innovative ideas</p>
            </div>
          </div>
          ...
          <div className="glass p-6 rounded-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-xl font-bold text-white">2.4M Credits Pledged</p>
              <p className="text-xs text-slate-400">Securing next-generation projects</p>
            </div>
          </div>
          ...
          <div className="glass p-6 rounded-2xl flex items-center gap-4">
            ...
            <div>
              <p className="text-xl font-bold text-white">99.2% Payout Success</p>
              <p className="text-xs text-slate-400">Escrow and bank withdrawal approvals</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex justify-between items-end mb-8 border-b border-white/5 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Backed by the community this week</p>
          </div>
          <Link href="/campaigns" className="text-xs font-semibold text-indigo-400 hover:underline">
            View all campaigns
          </Link>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 border-t border-white/5">
        <div className="text-center max-w-xl mx-auto space-y-2 mb-12">
          <h2 className="text-2xl font-extrabold text-white">Success Stories</h2>
          <p className="text-xs text-slate-400">Hear from our community of creators and supporters</p>
        </div>
          ...
          {testimonials.map((t, idx) => (
            <div key={idx} className="glass p-6 rounded-2xl flex flex-col justify-between space-y-4">
              ...
              <div className="flex items-center gap-2.5 pt-3 border-t border-white/5">
                <img src={t.avatar} alt={t.name} className="h-8 w-8 rounded-full border border-white/10" />
                <div>
                  <h4 className="text-xs font-bold text-white">{t.name}</h4>
                  <p className="text-[9px] text-slate-500 uppercase tracking-wider">{t.role}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="mx-auto max-w-3xl px-4 py-16 border-t border-white/5">
        <div className="text-center space-y-2 mb-10">
          <h2 className="text-2xl font-extrabold text-white flex items-center justify-center gap-1.5">
            <Award className="h-5 w-5 text-indigo-400" />
            <span>Frequently Asked Questions</span>
          </h2>
          <p className="text-xs text-slate-400">Got questions? We have answers.</p>
        </div>
          ...
          {faqs.map((faq, idx) => (
            <div key={idx} className="glass rounded-xl overflow-hidden transition-colors">
              <button
                ...
                className="w-full flex items-center justify-between p-4 font-semibold text-sm text-left hover:text-indigo-400 transition"
              >
                <span>{faq.q}</span>
                <ChevronDown
                  className={`h-4 w-4 text-slate-500 transition-transform duration-300 ${
                    activeFaq === idx ? 'transform rotate-180 text-indigo-400' : ''
                  }`}
                />
              </button>
              {activeFaq === idx && (
                <div className="p-4 pt-0 text-xs text-slate-400 leading-relaxed border-t border-white/5 mt-2 bg-slate-900/10">
                  {faq.a}
                </div>
              )}
            </div>
